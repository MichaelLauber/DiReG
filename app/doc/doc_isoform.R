div(
  ...
  p("While the human genome possesses approximately 1800 transcription factor (TF) genes, 
  ...
  p("Current transdifferentiation protocols often treat each TF as a single entity, 
    ...
    In their study, the authors transduced hESCs with their library of TF isoforms 
    and performed single cell RNA sequencing on these cells.Then two computational approaches
    diffusion and RNA velocity, were used to order TF-overexpressing cells along a 
  ...
  p("We allow the user to display these result for all TF isoforms of their input TFs. 
    ...
    significant differences between the splice variants. Furthermore, TF with a negative differentiation
    potential might not be good candidates to start with. Also we think that for users who have 
    a start cell other than hESC, might benefit from exploring the data. TFs with great variation 
    within its isoforms might make the isoforms worth to be tested in seperate experiments."),
